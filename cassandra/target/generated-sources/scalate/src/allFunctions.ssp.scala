/* NOTE this file is autogenerated by Scalate : see http://scalate.fusesource.org/ */

import _root_.scala.collection.JavaConversions._
import _root_.org.fusesource.scalate.support.TemplateConversions._
import _root_.org.fusesource.scalate.util.Measurements._

object $_scalate_$allFunctions_ssp {
  def $_scalate_$render($_scalate_$_context: _root_.org.fusesource.scalate.RenderContext): Unit = {
    ;{
      implicit val context: org.fusesource.scalate.DefaultRenderContext = $_scalate_$_context.attribute("context")
      import context._
      
      
      $_scalate_$_context << ( "\n\n" );
      import       org.apache.zeppelin.cassandra.MetaDataHierarchy._

      $_scalate_$_context << ( "\n" );
      import       java.util.UUID

      $_scalate_$_context << ( "\n" );
      ;{
        val allFunctions: Map[(UUID, String), Seq[FunctionSummary]] = $_scalate_$_context.attribute("allFunctions")
        $_scalate_$_context << ( "\n<div class=\"container\">\n\n    <div class=\"row\">\n        <div class=\"panel-group\" role=\"tablist\" aria-multiselectable=\"true\">\n            " );
        for (        ((ksId,ksName), functions) <- allFunctions
) {
          $_scalate_$_context << ( "            <div class=\"panel panel-default\">\n                <div class=\"panel-heading\" role=\"tab\">\n                    <h4 class=\"panel-title\">\n                        <a role=\"button\" data-toggle=\"collapse\" data-target=\"#" );
          $_scalate_$_context <<< (           ksId
 );
          $_scalate_$_context << ( "\" aria-expanded=\"false\">\n                            <span class=\"text-danger\"><span class=\"glyphicon glyphicon-folder-open\"></span>&nbsp;&nbsp;" );
          $_scalate_$_context <<< (           ksName
 );
          $_scalate_$_context << ( "</span>\n                        </a>\n                    </h4>\n                </div>\n                <div id=\"" );
          $_scalate_$_context <<< (           ksId
 );
          $_scalate_$_context << ( "\" class=\"panel-collapse collapse\" role=\"tabpanel\">\n                    <div class=\"panel-body\">\n                        <div class=\"row\">\n                            <div class=\"col-md-2\"></div>\n                            <div class=\"col-md-8 col-offset-md-2 table-responsive table-bordered\">\n                                <table class=\"table\">\n                                    <thead>\n                                        <tr>\n                                            <th style=\"text-align:right\">Function</th>\n                                            <th style=\"text-align:left\">Return Type</th>\n                                        </tr>\n                                    </thead>\n                                    <tbody>\n                                        " );
          for (          function <- functions
) {
            $_scalate_$_context << ( "                                        <tr class=\"text-success\">\n                                            <td style=\"text-align:right\">" );
            $_scalate_$_context <<< (             function.name + function.arguments.mkString("(",", ", ")")
 );
            $_scalate_$_context << ( "</td>\n                                            <td style=\"text-align:left\">" );
            $_scalate_$_context <<< (             function.returnType
 );
            $_scalate_$_context << ( "</td>\n                                        </tr>\n                                        " );
          }
          $_scalate_$_context << ( "                                    </tbody>\n                                </table>\n                            </div>\n                            <div class=\"col-md-2\"></div>\n                        </div>\n                    </div>\n                </div>\n            </div>\n            " );
        }
        $_scalate_$_context << ( "        </div>\n    </div>\n</div>" );
      }
    }
  }
}


class $_scalate_$allFunctions_ssp extends _root_.org.fusesource.scalate.Template {
  def render(context: _root_.org.fusesource.scalate.RenderContext): Unit = $_scalate_$allFunctions_ssp.$_scalate_$render(context)
}
