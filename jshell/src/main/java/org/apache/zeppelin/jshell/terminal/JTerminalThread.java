...
public class JTerminalThread extends Thread {
    private static final Logger LOGGER = LoggerFactory.getLogger(JTerminalThread.class);

    private Server jettyServer = new Server();

    private int port = 0;

    public JTerminalThread(int port) {
        this.port = port;
    }
    ...
    public void run() {
        ...
        ClassLoader clazz = JTerminalThread.class.getClassLoader();
        ...
        try {
            ...
            container.addEndpoint(JTerminalSocket.class);
            jettyServer.start();
            jettyServer.join();
        } catch (Exception e) {
