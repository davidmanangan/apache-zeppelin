...
import org.apache.zeppelin.jshell.terminal.*;
...
public class JShellTerminalInterpreter extends KerberosInterpreter {
    ...
    public JShellTerminalInterpreter(Properties property) {
        super(property);
    }

    private JTerminalThread terminalThread = null;
    ...
    public static final String TERMINAL_SOCKET_STATUS = "JSHELL_TERMINAL_SOCKET_STATUS";
    public static final String TERMINAL_SOCKET_CONNECT = "JSHELL_TERMINAL_SOCKET_CONNECT";
    public static final String TERMINAL_SOCKET_CLOSE = "JSHELL_TERMINAL_SOCKET_CLOSE";
    public static final String TERMINAL_SOCKET_ERROR = "JSHELL_TERMINAL_SOCKET_ERROR";
    ...
    public void close() {
        ...
        if (null != terminalThread) {
            JTerminalManager.getInstance().cleanIntpContext(intpContext.getNoteId());
    ...
    public InterpreterResult internalInterpret(String cmd, InterpreterContext context) {
        ...
        JTerminalManager.getInstance().setInterpreterContext(context);

        if (null == terminalThread) {
            try {
                terminalPort = RemoteInterpreterUtils.findRandomAvailablePortOnAllLocalInterfaces();
                terminalThread = new JTerminalThread(terminalPort);
    ...
    public void createTerminalDashboard(String noteId, String paragraphId, int port) {
        ...
        jinjaParams.put("JSHELL_TERMINAL_SERVER_URL", terminalServerUrl);
    ...
    protected boolean runKerberosLogin() {
        try {
            ...
            JTerminalManager.getInstance().runCommand(kinitCommand);
            return true;
        } catch (Exception e) {
